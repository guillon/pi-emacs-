...
(ert-deftest pi-session-info-recovers-from-malformed-project-metadata ()
  (pi-test-with-temp-home-project (root)
    (let ((pi-session-mode 'project))
      (with-temp-file (pi--project-session-meta-file root)
...
(ert-deftest pi-dispatch-uses-selected-backend-and-finishes-sync-responses ()
  (let ((calls nil)
    ...
    (cl-letf (((symbol-function 'pi--prepare-output)
      ...
      (should (equal (car (last calls)) '("check" (:root "/tmp/pi-dispatch" :file nil :mode text-mode :code "body" :session-info (:display "none") :kind ask))))
      (should (member 'prepare calls))
      (should (member 'redisplay calls))
      (should (member '(append "backend result") calls))
      (should (member '(finish ask) calls))
      (should (member '("request" (:root "/tmp/pi-dispatch" :file nil :mode text-mode :code "body" :session-info (:display "none") :kind ask) "Question" "minimal")
                      calls)))))

(ert-deftest pi-dispatch-aborts-before-preparing-output-when-backend-check-fails ()
  (let ((prepared nil)
        (source '(:root "/tmp/pi-dispatch"
                  :file nil
                  :mode text-mode
    ...
    (cl-letf (((symbol-function 'pi--prepare-output)
              ...
              ((symbol-function 'pi--call-backend)
               (lambda (name &rest _args)
                 (when (equal name "check")
                   (user-error "busy")))))
      (should-error (pi--dispatch source "Question" "minimal" 'ask)
                    :type 'user-error)
      (should-not prepared))))
...
(ert-deftest pi-new-session-preserves-fallback-to-none-mode ()
  (let ((prepared nil)
        (appended nil)
        (finished nil)
        (backend-source nil))
    (cl-letf (((symbol-function 'pi--project-root)
               (lambda () "/tmp/outside-home-project"))
              ((symbol-function 'pi--prepare-output)
               (lambda (source question kind)
                 (setq prepared (list source question kind))))
              ((symbol-function 'pi--append-output)
               (lambda (_root text)
                 (setq appended text)))
              ((symbol-function 'pi--finish-output)
               (lambda (source kind)
                 (setq finished (list source kind))))
              ((symbol-function 'pi--call-backend)
               (lambda (name source)
                 (setq backend-source (list name source)))))
      (let ((pi-session-mode 'project))
        (pi-new-session))
      (should (eq (plist-get (plist-get (car prepared) :session-info) :effective-mode) 'none))
      (should (equal (cadr prepared) "Start a fresh session"))
      (should (equal (caddr prepared) 'session))
      (should (string-match-p "fallback to none" appended))
      (should (eq (cadr finished) 'session))
      (should (equal (car backend-source) "new-session")))))

(ert-deftest pi-open-dispatches-to-open-backend ()
  (let ((selected-root nil)
        (backend-source nil))
    (cl-letf (((symbol-function 'pi--project-root)
               (lambda () "/tmp/pi-open-project"))
              ((symbol-function 'pi--select-output-buffer)
               (lambda (root)
                 (setq selected-root root)))
              ((symbol-function 'pi--call-backend)
               (lambda (name source)
                 (setq backend-source (list name source)))))
      (let ((pi-session-mode 'none))
        (pi-open))
      (should (equal selected-root "/tmp/pi-open-project"))
      (should (equal (car backend-source) "open"))
      (should (equal (plist-get (cadr backend-source) :root) "/tmp/pi-open-project")))))

(ert-deftest pi-abort-dispatches-to-abort-backend ()
  (let ((backend-source nil))
    (cl-letf (((symbol-function 'pi--project-root)
               (lambda () "/tmp/pi-abort-project"))
              ((symbol-function 'pi--call-backend)
               (lambda (name source)
                 (setq backend-source (list name source)))))
      (let ((pi-session-mode 'none))
        (pi-abort))
      (should (equal (car backend-source) "abort"))
      (should (equal (plist-get (cadr backend-source) :root) "/tmp/pi-abort-project")))))
